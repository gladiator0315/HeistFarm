    ...
    function PageYep:addPage(pagename,scrollsize,visible,elementspacing)
        ...
        function PageElements:addLabel(labelname,labelinfo)
            ...
            local wrapper = {
                ...
                Set = function(_, labelTitleText, labelInfoText)
                    if labelTitleText then
                        LabelTitle.Text = labelTitleText
                    end
                    if labelInfoText then
        ...
        function PageElements:addToggle(togglename,callback)
            ...
            local function updateToggleState(state)
                ...
                if ToggleEnabled then 
                    ToggleFrame.BackgroundColor3 = Color3.fromRGB(39, 149, 32)
            ...
            ToggleButton.MouseButton1Down:Connect(function()
                updateToggleState(not ToggleEnabled)
            end)
        ...
        function PageElements:addSlider(slidername,minvalue,maxvalue,step,callback)
            ...
            local function updateSlider(position)
                ...
                local ratio = math.clamp((position - buttonPos) / buttonSize, 0, 1)
            
                local newValue = minvalue + (maxvalue - minvalue) * ratio
                newValue = math.floor(newValue / step + 0.5) * step
                currentValue = newValue
            
                SliderTrail.Size = UDim2.new(ratio, 0, 1, 0)
